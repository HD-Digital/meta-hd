...
class Channelnumber:
	...
	def prikaz(self):
		...
		if config.plugins.VFD_ini.showClock.value == 'True' or config.plugins.VFD_ini.showClock.value == 'True_All' or config.plugins.VFD_ini.showClock.value == 'True_Switch' and self.dvb_service == "dvb":
...
class VFD_INISetup(ConfigListScreen, Screen):
	def __init__(self, session, args = None):
		...
		self.skin = """
			<screen position="center,center" size="500,210" title="LCD Display Setup" >
		...
		self.setTitle(_("Control 7 segment LCD display"))
	...
	def createSetup(self):
		self.list = []
		self.list.append(getConfigListEntry(_("Show on LCD"), config.plugins.VFD_ini.showClock))
		if config.plugins.VFD_ini.showClock.value != "Off":
